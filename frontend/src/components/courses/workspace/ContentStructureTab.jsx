...
const ContentStructureTab = ({
  ...
  const handleInlineCreate = useCallback((parentNode, contentType) => {
    // Use modals for module and lesson creation
    if (contentType === 'module') {
      setShowAddModuleModal(true);
    } else if (contentType === 'lesson') {
      setSelectedModule(parentNode.data);
      setShowAddLessonModal(true);
    } else if (contentType === 'quiz') {
      // Use quiz modal for quiz creation - no fallback allowed
      onOpenQuizModal();
    } else {
      // Keep panel for other content types (session, etc.)
      setCreateContext({
        parentNode,
        contentType,
  ...
  // Handle lesson creation from the new modal
  const handleLessonSubmit = useCallback(async (lessonData) => {
    try {
      await onCreateContent('lesson', lessonData, selectedModule.id);
  ...
  }, [onCreateContent, onRefresh, selectedModule]);

  // Handle lesson modal close
  ...
  // Handle lesson edit
  const handleLessonEdit = useCallback((lessonNode) => {
    setSelectedLesson(lessonNode.data);
    setShowEditLessonModal(true);
  }, []);

  // Handle lesson edit submission
  const handleLessonEditSubmit = useCallback(async (lessonData) => {
    try {
      // Use PUT/UPDATE API instead of POST/CREATE
      await API.lessons.updateLesson(selectedLesson.id, lessonData);
  ...
  }, [onRefresh, selectedLesson]);

  // Handle lesson edit modal close
  const handleLessonEditCancel = useCallback(() => {
    setShowEditLessonModal(false);
    setSelectedLesson(null);
  }, []);
  ...
  return (
    <div className="content-structure-tab">
      ...
      {/* Add Lesson Modal - NEW: Centered modal instead of right-side panel */}
      <AddLessonModal
      ...
      <AddLessonModal
        ...
        moduleId={selectedLesson?.moduleId || selectedLesson?.module_id}
